...
//===----------------------------------------------------------------------===//
//
//  This file defines schir::ContinuationStack
//
//===----------------------------------------------------------------------===//

#ifndef LLVM_SCHIR_CONTINUATION_STACK_H
#define LLVM_SCHIR_CONTINUATION_STACK_H
...
#ifndef SCHIR_STACK_SIZE
#define SCHIR_STACK_SIZE 1024 * 1024
#endif
...
class ContinuationStack {
  static_assert(SCHIR_STACK_SIZE > 0, "SCHIR_STACK_SIZE must be valid");
  std::vector<char> Storage;
  llvm::SmallVector<Value, 8> ApplyArgs; // includes the callee
  schir::Lambda* Top;
  schir::Lambda* Bottom; // not a valid Lambda but still castable to Value

  class DWind {
  public:
    schir::Vector* Vec = nullptr;
    DWind() = default;
...
public:
  ...
  schir::Value getCallee() {
    assert(ApplyArgs[0] && "callee must not be null");
    return ApplyArgs[0];
  }
  schir::ValueRefs getCallArgs() {
    return schir::ValueRefs(ApplyArgs).drop_front();
  }
  schir::ValueRefs getCaptures() {
    return cast<Lambda>(ApplyArgs[0])->getCaptures();
  }
  schir::Value getCapture(unsigned I) {
    return cast<Lambda>(ApplyArgs[0])->getCapture(I);
  }
  schir::Value getCurrentResult() {
  ...
  void PushCont(Fn const& F, llvm::ArrayRef<schir::Value> Captures = {}) {
    auto FnData = schir::createOpaqueFn(F);
  ...
  void PushCont(schir::Value Callable) {
    PushCont([](Derived& Context, ValueRefs Args) {
      schir::Value Callable = Context.getCapture(0);
  ...
  void RestoreStack(schir::String* Buffer) {
