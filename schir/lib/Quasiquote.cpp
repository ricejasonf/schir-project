...
//===----------------------------------------------------------------------===//
//
//  This file defines schir::Quasiquoter for the contextual processing of
//  quasiquote syntax
//
//===----------------------------------------------------------------------===//
...
//                (same as `cadr`)
schir::Value GetSingleSyntaxArg(schir::Pair* P) {
  // P->Car is the syntactic keyword
  schir::Pair* P2 = llvm::dyn_cast<schir::Pair>(P->Cdr);
  if (P2 && llvm::isa<schir::Empty>(P2->Cdr)) {
    return P2->Car;
  }
  return nullptr;
}
...
class Quasiquoter : private ValueVisitor<Quasiquoter, schir::Value> {
  friend class ValueVisitor<Quasiquoter, Value>;
  schir::OpGen& OpGen;

public:

  Quasiquoter(schir::OpGen& OG)
    : OpGen(OG)
  { }

  // <quasiquotation>
  mlir::Value Run(schir::Value Input) {
    bool Rebuilt = false;
    // <quasiquotation 1>
    schir::Value Result = Visit(Input, Rebuilt, /*Depth=*/1);
...
private:
  ...
  mlir::Operation* createSplice(SourceLocation Loc, schir::Value X,
                                schir::Value Y) {
    mlir::Value ValX = createValue(X);
    mlir::Value ValY = createValue(Y);
  ...
  schir::Value VisitValue(Value V, bool& Rebuilt, int Depth) {
    return V;
  }

  // <qq template D>
  schir::Value HandleQQTemplate(Value V, bool& Rebuilt, int Depth) {
  ...
  // <quasiquotation D>
  schir::Value HandleQuasiquote(Pair* P, bool& Rebuilt, int Depth) {
    ...
    schir::Value Result = Visit(Input, Rebuilt, Depth);
    if (!Rebuilt) return P;
  ...
  // <unquotation D>
  schir::Value HandleUnquote(Pair* P, bool& Rebuilt, int Depth) {
    Value Input = GetSingleSyntaxArg(P);
    if (!Input) return setError("invalid unquote syntax", P);

    schir::Value Result = HandleQQTemplate(Input, Rebuilt, Depth - 1);
    if (!Rebuilt) return P;
  ...
  schir::Value HandleUnquoteSplicing(Pair* Parent, Pair* P, Value Next,
    ...
    schir::Value NextResult = Visit(Next, NextRebuilt, Depth);
  ...
  // where D > 0
  schir::Value HandlePair(Pair* P, bool& Rebuilt, int Depth) {
    assert(Depth > 0 && "Depth cannot be zero here.");

    bool CdrRebuilt = false;
    bool CarRebuilt = false;
    schir::Value Cdr = Visit(P->Cdr, CdrRebuilt, Depth);
    schir::Value Car = Visit(P->Car, CarRebuilt, Depth);
  ...
  schir::Value VisitPair(Pair* P, bool& Rebuilt, int Depth) {
    assert(Depth > 0 && "Depth cannot be zero here.");
  ...
  schir::Value VisitVector(Vector* V, bool& Rebuilt, int Depth) {
    assert(Depth > 0 && "Depth cannot be zero here.");
    schir::Context& Context = OpGen.getContext();
    if (OpGen.CheckError()) return Undefined{};

    llvm::ArrayRef<schir::Value> Xs = V->getElements();
    if (Xs.empty())
      return V;

    // Convert the whole thing to a list.
    schir::Value Result = Visit(Context.CreateList(Xs), Rebuilt, Depth);
