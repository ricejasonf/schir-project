//===--- Parser.cpp - SchirScheme Language Parser --------------===//
//
...
//===----------------------------------------------------------------------===//
//
//  This file implements the Parser for SchirScheme.
//
//===----------------------------------------------------------------------===//
...
ValueResult Parser::Parse(schir::TokenKind Term) {
  if (CheckTerminator()) {
    return ValueEmpty();
  }
  PrimeToken(Term);
...
ValueResult Parser::ParseExpr(schir::Token const& StartTok) {
  switch (Tok.getKind()) {
  case tok::l_paren:
...
ValueResult Parser::ParseVector(Token const& StartTok,
  ...
  if (IsByteVector) {
    ...
    bool Valid = isa<schir::Int>(Result.get());
    if (Valid) {
      int32_t Byte = cast<schir::Int>(Result.get());
      if (Byte < 0 || Byte > 255)
        Valid = false;
    }
...
ValueResult Parser::ParseCharConstant() {
  ...
  if (TokData.size() == 1) C = TokData.front();
    ...
    schir::Value Result = Utf8View.drop_front();
    if (!Result || !Utf8View.empty())
      return SetError(Tok, "invalid character constant");
    C = uint32_t(llvm::cast<schir::Char>(Result));
  }
  ConsumeToken();
