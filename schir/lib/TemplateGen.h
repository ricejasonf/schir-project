...
//===----------------------------------------------------------------------===//
//
//  Define schir::TemplateGen mapping schir::Value to mlir::Value
...
public:
  ...
  void BuildTemplate(schir::Value Template) {
    {
      ...
      auto SyntaxOp = OpGen.create<schir::SyntaxOp>(
                          Keyword.getSourceLocation(), SyntaxFuncName);
      createRename(KeywordStr, SyntaxOp);
    }

    // Transform the syntax.
    schir::SourceLocation Loc = Template.getSourceLocation();
...
private:
  mlir::Value ExpandPack(schir::SourceLocation Loc,
                         schir::Value Car, schir::Value Cdr) {
  ...
  mlir::Value GetPatternVar(schir::Symbol* S) {
    if (OpGen.CheckError())
      return OpGen.createUndefined();
    schir::Context& Context = OpGen.getContext();
    schir::Value SCV = Context.Lookup(S).Value;
  ...
  mlir::Value MaybeCaptureExpandArg(schir::SourceLocation Loc,
                                    mlir::Value Result) {
    if (PackStack.empty())
      return Result;
    ...
    for (PackCaptureInfoTy PackCaptureInfo : PackStack) {
      ...
      if (Itr != CP->end()) {
        ...
        Result = Block->addArgument(SchirValueT, MLoc);
      }
    }
    return Result;
  }
...
public:
  Transformer(schir::OpGen& O, mlir::Value EnvArg, bool IsIr)
    : TemplateBase(O, EnvArg),
